    ...
    T: Copy,
{
    ...
    pub fn set_row<'a>(&'a mut self, row: usize, values: &'a Vec<T>) -> Result<&mut Self, usize> {
        if row >= self.rows {
            return Err(row);
        }
    ...
    pub fn set_columns(
        ...
        for (count, c) in columns.enumerate() {
    ...
    mod operations {
        ...
        mod add_scaled_column {
            ...
            fn invalid_column() {
                ...
                assert_eq!(m.add_scaled_column(2, 4, 1 / 1), Err(4));
            }
